...
async create(user: RequestUser, input: CreateCargoInput) {
  ...
  const pickup = await this.locations.resolve(input.pickup);
  const delivery = await this.locations.resolve(input.delivery);
 ...
 async update(user:RequestUser,id:string,input:UpdateCargoInput){
  ...
  return this.db.transaction(async client=>{
    ...
    const pickup=await this.locations.resolve(merged.pickup),delivery=await this.locations.resolve(merged.delivery);
    this.assertDifferentPlaces(pickup,delivery);
 ...
 async publish(user:RequestUser,id:string){await this.verification.assertIdentityVerified(user.id);return this.db.transaction(async client=>{const c=await this.lockOwned(client,id,user.id);if(c.status!=='draft')throw new ConflictException({code:'INVALID_CARGO_STATE',message:'Only draft cargo can be published'});const pickup=await this.locations.resolve({countryCode:c.pickup_country_code,cityId:c.pickup_city_id,street:c.pickup_street_private??''});const delivery=await this.locations.resolve({countryCode:c.delivery_country_code,cityId:c.delivery_city_id,street:c.delivery_street_private??''});this.assertDifferentPlaces(pickup,delivery);await client.query("UPDATE cargo SET status='published',pickup_point=ST_SetSRID(ST_MakePoint($2,$3),4326)::geography,delivery_point=ST_SetSRID(ST_MakePoint($4,$5),4326)::geography,pickup_public_label=$6,delivery_public_label=$7,updated_at=now() WHERE id=$1",[id,pickup.lng,pickup.lat,delivery.lng,delivery.lat,pickup.publicLabel,delivery.publicLabel]);await this.matching.recomputeCargo(client,id);await this.audit(client,user,'cargo.published',id,{pickupGeocodingSource:pickup.geocodingSource,deliveryGeocodingSource:delivery.geocodingSource});return this.getWithClient(client,id);});}
 ...
 private assertDifferentPlaces(a:any,b:any){const sameCity=a.countryCode===b.countryCode&&a.cityId===b.cityId;const sameStreet=this.locations.normalizeStreet(a.street)===this.locations.normalizeStreet(b.street);if(sameCity&&sameStreet)throw new BadRequestException({code:'SAME_ADDRESS',message:'Pickup and delivery address must be different'});if(sameCity&&(!a.geocoded||!b.geocoded))throw new BadRequestException({code:'INTRACITY_ADDRESS_NOT_RESOLVED',message:'Для доставки в межах одного міста вкажіть точні адреси, які можна знайти на карті'});}
