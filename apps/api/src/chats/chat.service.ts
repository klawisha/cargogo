...
export class ChatService{
  ...
  private state(row:any){
    ...
    return{readOnly:terminal||expired,closesAt:closesAt?.toISOString()??null,archived:terminal||expired};
  }

  async listMine(user:RequestUser){
    const r=await this.db.query<any>(`SELECT dc.deal_id,d.status,d.completed_at,d.delivery_verified_at,d.sender_id,d.driver_id,c.title AS cargo_title,
      ...
      WHERE d.sender_id=$1 OR d.driver_id=$1 ORDER BY COALESCE(lm.created_at,dc.created_at) DESC LIMIT 100`,[user.id]);
  ...
  async send(user:RequestUser,dealId:string,body:string){
    return this.db.transaction(async(client)=>{
      const dealResult=await client.query<any>('SELECT id,sender_id,driver_id,status,completed_at,delivery_verified_at FROM deal WHERE id=$1 FOR UPDATE',[dealId]);
  ...
  private async requireConversation(userId:string,dealId:string){
    const r=await this.db.query<any>(`SELECT dc.id AS conversation_id,d.sender_id,d.driver_id,d.status,d.completed_at,d.delivery_verified_at FROM deal_conversation dc JOIN deal d ON d.id=dc.deal_id WHERE d.id=$1`,[dealId]);
    const row=r.rows[0];if(!row)throw new NotFoundException({code:'CHAT_NOT_FOUND',message:'Chat not found'});if(row.sender_id!==userId&&row.driver_id!==userId)throw new ForbiddenException({code:'CHAT_FORBIDDEN',message:'Not your deal'});return {conversationId:row.conversation_id,status:row.status,completed_at:row.completed_at,delivery_verified_at:row.delivery_verified_at};
  }
}
