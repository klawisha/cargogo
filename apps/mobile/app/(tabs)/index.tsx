...
import { loadDriverReadiness, type DriverReadiness } from '@/readiness/driver-readiness';
...
export default function Home() {
  ...
  const [readiness,setReadiness] = useState<DriverReadiness|null>(null);
  const liveVersion=useLiveVersion('cargo','trip','deals');const{connected}=useLive();

  const load = useCallback(async () => {
    setLoading(true); setError('');
    try {
      const [cargoResponse,tripResponse,currentResponse,readinessData] = await Promise.all([apiFetch('/cargo/mine'),apiFetch('/trips/mine'),apiFetch('/trips/current'),loadDriverReadiness().catch(()=>null)]);
      ...
      setCargo(cargoData); setTrips(tripData); setCurrentTrip(currentData); if(readinessData)setReadiness(readinessData);
    } catch(e) { setError(e instanceof Error ? e.message : 'Помилка'); }
    finally { setLoading(false); }
  },[]);

  useEffect(() => { void load(); },[load,liveVersion]);
  useFocusEffect(useCallback(()=>{void loadDriverReadiness().then(setReadiness).catch(()=>undefined)},[]));
  ...
  return <Screen><View style={s.header}><View><View style={s.brandRow}><BrandLogo compact/><View style={[s.live,!connected&&{opacity:.45}]}><PulseDot color={colors.success} size={5}/><Text style={s.liveText}>{connected?'LIVE':'SYNC'}</Text></View></View><Text style={s.kicker}>SMART ROUTES · VERIFIED DEALS · SECURE PAYMENT</Text></View><Pressable onPress={()=>router.push('/(tabs)/profile')} hitSlop={12} style={({pressed})=>[s.avatar,pressed&&s.pressed]}><Text style={s.avatarText}>{user?.displayName.slice(0,2).toUpperCase()}</Text></Pressable></View>
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{paddingBottom:124}}>
      <View style={s.trust}><Text style={s.eye}>IDENTITY</Text><Text style={s.meta}>{readiness?.identityReady?'VERIFIED':'VERIFICATION REQUIRED'}</Text></View>

      {readiness&&(!readiness.identityReady||!readiness.ready)&&<View style={s.readiness}>
        <View style={s.readinessHead}><View><Text style={s.eye}>ЩО ПОТРІБНО ДЛЯ СТАРТУ</Text><Text style={s.readinessTitle}>Окремо для відправника і водія</Text></View></View>

        <Pressable onPress={()=>readiness.identityReady?router.push('/create-cargo'):router.push('/verification')} style={({pressed})=>[s.roleChecklist,pressed&&s.pressed]}>
          <View style={s.roleChecklistHead}><View><Text style={s.roleLabel}>ВІДПРАВИТИ / ПЕРЕВЕЗТИ ВАНТАЖ</Text><Text style={s.roleTitle}>{readiness.identityReady?'ГОТОВО ДО ПУБЛІКАЦІЇ':'ПОТРІБЕН ПІДТВЕРДЖЕНИЙ ID'}</Text></View><Text style={s.readinessArrow}>→</Text></View>
          <ReadinessLine done={readiness.identityReady} title="ID-картка або паспорт" meta={readiness.identityReady?'Особу підтверджено — вантажі можна публікувати.':'Потрібно лише підтвердити особу. Права та автомобіль відправнику не потрібні.'}/>
        </Pressable>

        <Pressable onPress={()=>router.push('/driver-readiness')} style={({pressed})=>[s.roleChecklist,pressed&&s.pressed]}>
          <View style={s.roleChecklistHead}><View><Text style={s.roleLabel}>ЇХАТИ ЯК ВОДІЙ</Text><Text style={s.roleTitle}>{readiness.ready?'ГОТОВО ДО ПОЇЗДОК':`${readiness.completed}/${readiness.total} КРОКИ ВИКОНАНО`}</Text></View><Text style={s.readinessArrow}>→</Text></View>
          <ReadinessLine done={readiness.identityReady} title="ID-картка або паспорт" meta="Підтвердження особи"/>
          <ReadinessLine done={readiness.licenseReady} title="Водійське посвідчення" meta="Підтверджені права водія"/>
          <ReadinessLine done={readiness.vehicleReady} title="Автомобіль + техпаспорт" meta="Щонайменше один verified автомобіль"/>
          <ReadinessLine done={readiness.carrierReady} title="Режим перевізника" meta={readiness.carrier.mode==='professional'?'CargoGo Pro має пройти перевірку':'Прийміть правила попутного перевізника'}/>
        </Pressable>
        <Text style={s.readinessHint}>Для відправки вантажу не потрібні водійські права чи автомобіль. Вони потрібні лише для створення поїздки як водій.</Text>
      </View>}

      <FadeInView delay={35}><View style={s.map}><View style={s.mapTop}><Text style={s.mapLabel}>ROUTE ENGINE · ROAD LIVE</Text><Text style={s.build}>LIVE</Text></View><RoutePreview origin={activeTrip?.origin.publicLabel??'СТАРТ'} destination={activeTrip?.destination.publicLabel??'ФІНІШ'} distance={activeTrip?distanceLabel(activeTrip.route.distanceM):undefined} durationS={activeTrip?.route.durationS} coordinates={activeTrip?.route.coordinates??[]}/><Text style={s.routeTitle}>{activeTrip ? `${activeTrip.origin.publicLabel} → ${activeTrip.destination.publicLabel}` : 'Створіть напрямок'}</Text><Text style={s.meta}>{activeTrip ? `${distanceLabel(activeTrip.route.distanceM)} · ${activeTrip.route.quality.toUpperCase()}` : 'PostGIS corridor matching готовий до першої поїздки'}</Text>{activeTrip&&<Pressable onPress={()=>router.push({pathname:'/trip/[id]',params:{id:activeTrip.id}})} style={s.currentMatch}><View><Text style={s.eye}>ПОТОЧНИЙ MATCH</Text><Text style={s.currentMatchText}>{activeTrip.status==='published'?'Відкрити актуальні вантажі':'Маршрут збережено · зараз неактивний'}</Text></View><Text style={s.action}>→</Text></Pressable>}</View></FadeInView>
