...
export default function Home() {
  ...
  return <Screen><View style={s.header}><View><View style={s.brandRow}><BrandLogo compact/><View style={[s.live,!connected&&{opacity:.45}]}><PulseDot color={colors.success} size={5}/><Text style={s.liveText}>{connected?'LIVE':'SYNC'}</Text></View></View><Text style={s.kicker}>SMART ROUTES · VERIFIED DEALS · SECURE PAYMENT</Text></View><Pressable onPress={()=>router.push('/(tabs)/profile')} hitSlop={12} style={({pressed})=>[s.avatar,pressed&&s.pressed]}><Text style={s.avatarText}>{user?.displayName.slice(0,2).toUpperCase()}</Text></Pressable></View>
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{paddingBottom:124}}>
      ...
      {readiness&&!readiness.ready&&<Pressable onPress={()=>router.push('/driver-readiness')} style={({pressed})=>[s.readiness,pressed&&s.pressed]}>
        <View style={s.readinessHead}><View><Text style={s.eye}>ГОТОВНІСТЬ ДО ПЕРЕВЕЗЕНЬ</Text><Text style={s.readinessTitle}>{readiness.completed}/{readiness.total} кроки виконано</Text></View><Text style={s.readinessArrow}>→</Text></View>
        {!readiness.identityReady&&<ReadinessLine done={false} title="Підтвердіть особу" meta="ID verification потрібна до створення маршруту"/>}
        {readiness.identityReady&&!(readiness.licenseReady&&readiness.vehicleReady)&&<ReadinessLine done={false} title="Підтвердіть водія та авто" meta="Посвідчення + щонайменше один verified vehicle"/>}
        {!readiness.carrierReady&&<ReadinessLine done={false} title="Прийміть правила перевізника" meta={readiness.carrier.mode==='professional'?'Завершіть перевірку CargoGo Pro':'Прийміть Casual Driver Rules або оберіть CargoGo Pro'}/>}
        <Text style={s.readinessHint}>Відкрийте чекліст — CargoGo одразу приведе до потрібного кроку.</Text>
      </Pressable>}
