...
export default function DealDetail() {
  ...
  return <Screen><ScrollView contentContainerStyle={{paddingBottom:40}} showsVerticalScrollIndicator={false}>
    ...
    {deal&&<>
      ...
      <View style={s.party}><Text style={s.label}>ВІДПРАВНИК</Text><Text style={s.value}>{deal.sender.displayName} · {deal.sender.verificationStatus}</Text><Text style={s.label}>ПЕРЕВІЗНИК</Text><Text style={s.value}>{deal.driver.displayName} · {deal.driver.verificationStatus}</Text></View>

      <Pressable onPress={()=>router.push({pathname:'/chat/[dealId]',params:{dealId:String(id)}})} style={s.outline}><Text style={s.outlineText}>ВІДКРИТИ ЧАТ УГОДИ</Text><Text style={s.outlineText}>→</Text></Pressable>
